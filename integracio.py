...
def main(page: Page):
    ...
    page.fonts = {
           ...
           "Main": "fonts/Mainlux-Light.otf",
           "Stres": "fonts/strenuous bl.otf",
    ...
    async def animate_center(e):
        pass   
    size_title = page.height*0.8*0.1 / 2
    if size_title >= 28:
        size_title = 28
                    ...
                    Image(
                        ...
                        filter_quality="HIGH",  
                           # image_fit = "FIT_HEIGHT",
                            #width=page.width,  # Asegurar que la imagen tenga un ancho específico # Asegurar que la imagen tenga una altura específica
                        )
                    ]
                )
...
#Definirem aqui tots els components com a variables per a tal d'accedir-hi en qualsevol moment en el programa
                ...
                for i in range(0,10):
                    ...
                    content=Column(
                        ...
                        controls=[
                            ...
                            Image(
                                src=f"https://picsum.photos/{round(page.width * 0.8)}/{round((page.height * 0.8 )* 0.65)}",
                                border_radius = 10+((page.height*0.8)*0.16),
                                width = page.width * 0.8, 
                                height = page.height * 0.8 * 0.65,
                                filter_quality="HIGH",  
                                # image_fit = "FIT_HEIGHT",
    ...
    async def scale_next_card():
        if len(cards) > 1:  # Check if there are cards left
            next_card = cards[0]
            next_card.scale = 0
            page.update()
    ...
    Tags_amunt = SafeArea(content=Tags_amunt, top=True)
    page.add(
        Tags_amunt,
        stack,
        botons, 
