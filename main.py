...
import os 
from flet_geolocator import GeolocatorPermissionStatus, Geolocator, GeolocatorConfiguration, GeolocatorPositionAccuracy
...
import datetime
from scripts.llocs import LLocs_sostenibles, Llocs, Llocs_info, Llocs_Yelp_info, Llocs_yelp
...
from src.context import AppCtx
from src.pages import home as home_page, error as error_page, favorits as favorits_page
from src.pages import categories as categories_page, lloc_especific as lloc_especific_page
from src.pages import info as info_page, ia as ia_page
from src.pages.configuracio import (
    menu as config_menu,
    historial as config_historial,
    tema as config_tema,
    privacitat as config_privacitat,
    idioma as config_idioma,
    config_near as config_near_page,
    sobre_app as config_sobre,
    ajuda as config_ajuda,
)

async def main(page: Page):
    APP_SESSIONS.setdefault(page, {})
    ctx = AppCtx(APP_SESSIONS, get_client_storage, logger)
    ...
    async def on_change_page(e):
        ...
        if page.route == '/':
            await home_page.build_view(page, ctx)
        elif page.route == '/error':
            await error_page.build_view(page, ctx)
        elif page.route == '/favorits':
            await favorits_page.build_view(page, ctx)
        elif page.route == '/configuracio':
            await config_menu.build_view(page, ctx)
        elif page.route == '/configuracio/historial':
            await config_historial.build_view(page, ctx)
        elif page.route == '/configuracio/tema':
            await config_tema.build_view(page, ctx)
        elif page.route == '/configuracio/politica_privacitat':
            await config_privacitat.build_view(page, ctx)
        elif page.route == '/configuracio/idioma':
            await config_idioma.build_view(page, ctx)
        elif page.route == '/configuracio/config_near':
            await config_near_page.build_view(page, ctx)
        elif page.route == '/configuracio/sobre_app':
            await config_sobre.build_view(page, ctx)
        elif page.route == '/configuracio/ajuda':
            await config_ajuda.build_view(page, ctx)
        elif page.route == '/info':
            await info_page.build_view(page, ctx)
        elif page.route == '/categories':
            await categories_page.build_view(page, ctx, Tags_amunt_safe)
        elif page.route == '/lloc_especific':
            await lloc_especific_page.build_view(page, ctx)
        elif page.route == '/ia':
            await ia_page.build_view(page, ctx)
    ...
    async def seguent(e):
        ctx.cards[0].offset = Offset(-4, 0)  
        page.update()
        await asyncio.sleep(0.15)  
        ctx.cards.remove(ctx.cards[0])
        logger.debug(f"Card {len(ctx.cards)} swiped left")
        await update_cards()
    ...
    async def guarda(e):
        ...
        saved_cards.append(dadesLlocs[ctx.index_photo_stack])
        saved_cards_images.append(images_request[ctx.index_photo_stack][0]) if images_request[index_photo_stack] != [] else saved_cards_images.append(images_request[index_photo_stack])
    ...
    async def mes_info(e):
        ...
        ctx.cards[0].animate_scale = Animation(550)
        ctx.cards[0].scale = 2
        ctx.cards[0].opacity = 0.1
        ctx.botons.animate_opacity = Animation(550)
        ctx.Tags_amunt.animate_opacity = Animation(550)
        ctx.botons.opacity = 0.12
        ctx.Tags_amunt.opacity = 0.12
        page.update()
        await asyncio.sleep(0.5)
        page.controls.clear()
        page.update()
    ...
    async def categ_chip_sel(e):
        ...
        if e.control.label.value == "AI":
            if e.control.selected and ctx.ai != 2:
                ctx.ai = 2
    ...
    async def mes_info_select(e):
        ...
        ctx.Tags_amunt.controls[len(ctx.Tags_amunt.controls) - 1].content.selected = False
...
#Definirem aqui tots els components com a variables per a tal d'accedir-hi en qualsevol moment en el programa
    ...
    # ── Assign UI components and early callbacks to ctx ─────────────────────────
    # Note: ctx.cards is already [] from AppCtx.__init__; update_cards fills it
    ctx.stack_cards = stack_cards
    ctx.botons = botons
    ctx.Tags_amunt = Tags_amunt
    ctx.images_saved = images_saved
    ctx.configuracio = configuracio
    ctx.size_botons = size_botons
    ctx.categories_list = categories_list
    ctx.categ_check_sel = categ_check_sel
    ctx.categ_chip_sel = categ_chip_sel
    ctx.view_pop = view_pop
    ctx.seguent = seguent
    ctx.guarda = guarda
    ctx.mes_info = mes_info
    ctx.config_nav = config_near
    ctx.mes_info_select = mes_info_select
    # ctx.update_cards and ctx.scale_next_card assigned below after their definitions
    ...
    async def update_cards():
        ...
        logger.debug(f"Variables globals - canvi: {ctx.canvi}, sostenible: {ctx.sostenible}, sostenible_2: {ctx.sostenible_2}")
        if ctx.canvi == True:
        ...
        if len(ctx.cards) == 0 or ctx.canvi == True:
                logger.info("Iniciant proces de demanar dades - Cards buides o canvi activat")
                #* Demanem les dades 
                logger.debug(f"index_photo_Stack: {ctx.index_photo_stack}")
                if ctx.canvi == True:
                    ctx.sostenible = True
                    ctx.sostenible_2 = True
                    logger.info(f"Reset variables - sostenible: {ctx.sostenible}, sostenible_2: {ctx.sostenible_2}")
                    logger.info(f"Llocs visitats abans del reset: {len(loc_visited)}")
                    dadesLlocs = []
                    ctx.cards.clear()

                ctx.index_photo_stack = -1
                ...
                ctx.Foursquare, ctx.Yelp, ctx.Sostenible_L = False, False, False
                ...
                for origen in ordre:
                    if origen == "SOSTENIBLE":
                        ...
                        ctx.Foursquare, ctx.Yelp, ctx.Sostenible_L = False, False, True
                        ...
                        ctx.Foursquare, ctx.Yelp, ctx.Sostenible_L = False, True, False
                        if "lloc_especific" in APP_SESSIONS[page]:
                            lloc_especific = APP_SESSIONS[page].get("lloc_especific")
                            if lloc_especific != "":
                        ...
                        ctx.Foursquare, ctx.Yelp, ctx.Sostenible_L = True, False, False
                        if "lloc_especific" in APP_SESSIONS[page]:
                            lloc_especific = APP_SESSIONS[page].get("lloc_especific")
                            if lloc_especific != "":
                ...
                ctx.sostenible_2 = False
                ctx.sostenible = False
                #dadesLlocs, loc_visited = llocs.dades()
                ...
                if dadesLlocs == "error 400" or dadesLlocs == "error 400 de l'API sostenible" or not isinstance(dadesLlocs, list):
                    ...
                    logger.info(f"EXIT: Llocs trobats: {len(dadesLlocs)} - Origen: {'Sostenible' if ctx.Sostenible_L else 'Yelp' if ctx.Yelp else 'Foursquare'}")
                    ...
                    for i in range(len(dadesLlocs)):
                        ...
                        async def esq(e): #Detecta que has fet click a l'esquerra 
                            ...
                            if index_photo <= 0:
                                    ctx.index_photo = len(images_request[ctx.index_photo_stack]) - 1 # Fa que sempre l'index sigui un número a dins de la llista i resta un, fent així que puguem navegar
                            else:
                                ctx.index_photo -= 1
                            img_principal.opacity = 0.1 #Animació d'opactiat, perquè l'usuari tingui més comoditat visual 
                            img_principal.update()
                            await asyncio.sleep(0.15) 
                            img_principal.src = images_request[ctx.index_photo_stack][ctx.index_photo] #Actualitza les fotos 
                            ...
                            img_esq.src = images_request[ctx.index_photo_stack][ctx.index_photo-1 if ctx.index_photo-1 >= 0 else (len(images_request[ctx.index_photo_stack])-1)]  
                            ...
                            img_dret.src = images_request[ctx.index_photo_stack][ctx.index_photo+1 if ctx.index_photo+1 <= (len(images_request[ctx.index_photo_stack])- 1) else 0] 
                        ...
                        async def dret(e): #Mateixos comentaris pero al reves
                            ...
                            img_dret = stack_cards.controls[0].content.content.controls[1].content.controls[2]
                            img_esq = stack_cards.controls[0].content.content.controls[1].content.controls[1]
                            if ctx.index_photo >= (len(images_request[ctx.index_photo_stack])- 1):
                                ctx.index_photo = 0
                            else:
                                ctx.index_photo += 1
                            img_principal.opacity = 0.1 #Animació d'opactiat, perquè l'usuari tingui més comoditat visual 
                            img_principal.update()
                            await asyncio.sleep(0.15) 
                            img_principal.src = images_request[ctx.index_photo_stack][ctx.index_photo] 
                            img_principal.opacity = 1
                            img_esq.src = images_request[ctx.index_photo_stack][ctx.index_photo-1 if ctx.index_photo-1 >= 0 else (len(images_request[ctx.index_photo_stack])-1)]  
                            img_dret.src = images_request[ctx.index_photo_stack][ctx.index_photo+1 if ctx.index_photo+1 <= (len(images_request[ctx.index_photo_stack])- 1) else 0]  
                            page.update()
        ...
        for card in ctx.cards:
            if not first_card_added:
                ...
                dadesLlocs = APP_SESSIONS[page].get("dadesLlocs")
                loc_visited.append(dadesLlocs[ctx.index_photo_stack])
                loc_visited_photos.append(images_request[ctx.index_photo_stack])
                ...
                if len(images_request[ctx.index_photo_stack]) > 1:
                    img_principal.src = images_request[ctx.index_photo_stack][0]
                    img_dret.src = images_request[ctx.index_photo_stack][1] 
                    img_esq.src = images_request[ctx.index_photo_stack][len(images_request[ctx.index_photo_stack]) - 1]
                    page.update()
                elif len(images_request[ctx.index_photo_stack]) == 1:
                    logger.debug("es 1")
                    img_principal.src = images_request[ctx.index_photo_stack][0]
                ...
                elif len(images_request[ctx.index_photo_stack]) == 0:
                    IconButton_dret.visible = False
                    IconButton_esq.visible = False
    ...
    # ── Late ctx assignments (functions defined after update_cards call) ────────
    ctx.update_cards = update_cards
    ctx.scale_next_card = scale_next_card
